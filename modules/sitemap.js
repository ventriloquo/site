  ...
  document.getElementById("sitemap").appendChild(
    tag("div", {},
      tag("h2", {},
        ...
        tag("span", {"style":"color: rgba(var(--ac-1), 1)"}, `${page_list.length  + posts.length}`),
        tag("br")),
      tag("br"),
      tag("ul", {"id":"page_list", "style":"white-space: pre; max-width: 100%; overflow-x: scroll"})
    )
  );
  
  for (const page of page_list) {
    const page_index = page_list.indexOf(page);
    document.getElementById("page_list").appendChild(tag("li", {"id":`page_item_${page_index}`}, tag("a", {"href":page.id}, page.id)));
  }
